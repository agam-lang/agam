...
**Status:** partial (Basic NVPTX pipeline, source-level `@gpu(...)` config parsing, host-device transfer helpers, shared-memory lowering, and LLVM host launch lowering complete; advanced capabilities pending)
...
- [x] Parse annotation argument lists and tuple literals so source-level `@gpu(threads=..., shared=..., grid=(...))` survives into sema/HIR/MIR
...
- [x] Parse and resolve fixed-size array types (`[T; N]`) so GPU params and shared allocations can lower as element-typed device buffers.
- [x] Run GPU kernel validation during HIR lowering and permit shared-memory plus pointer/array indexing paths that lower today.
...
- [x] Lower host calls to known `@gpu` functions into `Op::GpuKernelLaunch` using the current scalar launch contract (`grid.x`, block size, shared bytes)
...
- [x] Lower `Op::GpuKernelLaunch` in the LLVM host backend into concrete CUDA runtime IR with stack-packed kernel args plus `cudaLaunchKernel(...)`
...
- [x] Preserve typed GPU pointer ABI for reference-wrapped buffers (`&[T]`, `&[T; N]`, `&mut [T; N]`) instead of collapsing them to opaque pointers.
- [x] Support wide LLVM integer kernel types (`i128`, `i256`, `i512`) and matching pointer/shared-memory forms for GPU params and fixed-size array/shared-alloc lowering.
...
- [x] Preserve typed shared-memory lowering for reference-wrapped shared alloc targets (`&mut [T]`) instead of degrading them to opaque pointers.
...
## Test Results
- 35+ GPU-focused tests across parser/HIR/MIR/sema/codegen
