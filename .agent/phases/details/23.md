...
**Status:** completed (NVPTX pipeline, fixed-size array GPU memory typing, host-device transfer helpers, shared-memory lowering, and rich memory type classification complete)
...
- [x] Rich Memory Types (`GpuMemoryType` address space qualification: `addrspace(1)` global, `addrspace(3)` shared, `addrspace(4)` constant, `addrspace(5)` local, `noalias` restrict qualification).
