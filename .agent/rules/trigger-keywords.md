...
| **`"Recommend Feature"`** | Analyze current compiler capability gaps, evaluate T0–T6 catalog, suggest strategic algorithms, and add spec entries to the catalog/roadmap (do not directly modify Rust source code without explicit host permission). | **Stream 2 (Frontier Horizon Sync)** |
...
| **`"Inject Mathematical Models"`** | Analyze frontier papers, articles, and classical theorems (e.g., E-Graph term rewriting, Hankel moment systems, Kronecker Jacobians, representation graphs, resolvent purifications), synthesize new algorithms/mathematical models, add spec details to the catalog/roadmap for future implementation, and do NOT directly change code without proper permission of the host. | **Stream 2 (Frontier Theory & Algorithm Synthesis)** |
