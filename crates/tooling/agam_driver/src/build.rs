...
pub(crate) const LLVM_CLANG_ENV: &str = "AGAM_LLVM_CLANG";

pub(crate) const LLVM_BUNDLE_DIR_ENV: &str = "AGAM_LLVM_BUNDLE_DIR";
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum LlvmTargetPlatform {
    Windows,
    Linux,
    MacOs,
    Ios,
...
pub(crate) fn classify_llvm_target_platform(target: Option<&str>) -> LlvmTargetPlatform {
...
pub(crate) fn host_llvm_target_platform() -> LlvmTargetPlatform {
    if cfg!(windows) {
...
pub(crate) fn configured_llvm_clang_override() -> Option<String> {
    std::env::var(LLVM_CLANG_ENV)
        .ok()
        .filter(|value| !value.trim().is_empty())
}

pub(crate) fn llvm_driver_file_names() -> &'static [&'static str] {
    if cfg!(windows) {
        &["clang.exe", "clang++.exe"]
    } else {
        &["clang", "clang++"]
...
pub(crate) fn bundled_llvm_platform_dir() -> &'static str {
...
pub(crate) fn bundled_llvm_candidate_paths(root: &Path) -> Vec<PathBuf> {
...
pub(crate) fn discover_bundled_llvm_clang() -> Option<String> {
...
pub(crate) fn windows_vswhere_path() -> Option<PathBuf> {
    if !cfg!(windows) {
        return None;
    }
...
pub(crate) fn discover_visual_studio_installation_path() -> Option<PathBuf> {
...
pub(crate) fn visual_studio_llvm_candidate_paths(install_root: &Path) -> Vec<PathBuf> {
    vec![
        install_root
            .join("VC")
            .join("Tools")
...
pub(crate) fn standalone_windows_llvm_install_roots() -> Vec<PathBuf> {
    if !cfg!(windows) {
        return Vec::new();
    }
...
pub(crate) fn standalone_windows_llvm_candidate_paths(install_root: &Path) -> Vec<PathBuf> {
...
pub(crate) fn discover_standalone_windows_llvm_clang() -> Option<String> {
    if !cfg!(windows) {
        return None;
    }
...
pub(crate) fn discover_visual_studio_llvm_clang() -> Option<String> {
...
pub(crate) fn native_llvm_clang_candidates() -> Vec<String> {
...
pub(crate) fn resolve_native_llvm_command() -> Option<String> {
    native_llvm_clang_candidates()
        .into_iter()
        .find(|candidate| command_exists(candidate))
}

pub(crate) fn configured_llvm_clang() -> String {
