...
impl Parser {
    ...
    fn parse_annotations(&mut self) -> Result<Vec<Annotation>, ParseError> {
        ...
        while self.peek_kind() == TokenKind::At {
            ...
            if !name_parts.is_empty() {
                let name = Ident::new(
                    name_parts.join("."),
                    Span::new(self.peek().span.source_id, start, end_span),
                );
    ...
    fn parse_statement(&mut self) -> Result<Stmt, ParseError> {
        ...
        Ok(Stmt {
            id: self.node_id(),
            span: Span::new(self.peek().span.source_id, start, self.peek().span.end),
            kind,
        })
    }
    ...
    fn parse_const_stmt(&mut self) -> Result<StmtKind, ParseError> {
        ...
        Ok(StmtKind::Const {
            name: Ident::new(&name_tok.lexeme, name_tok.span),
            ty,
            value,
        })
    }
        ...
        match tok.kind {
            ...
            TokenKind::Identifier | TokenKind::Self_ => {
                    ...
                    && (self.peek_at(2) == TokenKind::Colon || self.peek_at(1) == TokenKind::RBrace)
                {
                    ...
                    while self.peek_kind() == TokenKind::Identifier {
                        ...
                        let value = if self.eat(TokenKind::Colon) {
                            ...
                            Expr {
                                id: self.node_id(),
                                span: fname_tok.span,
                                kind: ExprKind::Identifier(fname.clone()),
                            }
                        };
                    ...
                    return Ok(Expr {
                        id,
                        span: Span::new(tok.span.source_id, tok.span.start, end.end),
                ...
                if segments.len() == 1 {
                    Ok(Expr {
                        id,
                        span: tok.span,
                        kind: ExprKind::Identifier(segments.pop().unwrap()),
                    })
                } else {
                    Ok(Expr {
                        id,
        ...
        match op_tok.kind {
            ...
            TokenKind::LBrace => {
                ...
                let path = match &left.kind {
                    ExprKind::Identifier(ident) => Path {
                    ...
                    ExprKind::FieldAccess { object, field } => {
                        if let ExprKind::Identifier(obj_id) = &object.kind {
                            Path {
                                segments: vec![obj_id.clone(), field.clone()],
                                span: left.span,
                            }
                        } else {
                    ...
                    _ => {
                        return Err(
                            self.error("Expected identifier or path before '{'".to_string())
                        );
                    }
                };
        ...
        if tok.kind == TokenKind::Dyn {
            ...
            if self.peek_kind() == TokenKind::Identifier || self.peek_kind() == TokenKind::SelfType
            {
                ...
                return Ok(TypeExpr {
                    id,
                    span: tok.span,
                    kind: TypeExprKind::DynTrait(Box::new(inner)),
                    mode: TypeMode::Dynamic,
                });
            } else {
                return Ok(TypeExpr {
...
mod tests {
    ...
    fn test_float_literal() {
        let expr = parse_expr("3.125");
        if let ExprKind::FloatLiteral(n) = expr.kind {
            assert!((n - 3.125).abs() < 0.001);
        }
    }
    ...
    fn test_parse_fixed_array_type() {
        ...
        match &f.params[0].ty.kind {
            TypeExprKind::Array { element, size } => {
                ...
                assert!(matches!(size.kind, ExprKind::IntLiteral(64)));
            }
            other => panic!("expected fixed array type, found {:?}", other),
        }
    }
