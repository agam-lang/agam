...
pub enum Type {
    ...
    Int(IntSize),
    /// Unsigned integers: u8, u16, u32, u64, u128, u256, u512, usize
    UInt(IntSize),
...
impl TypeStore {
    pub fn new() -> Self {
        ...
        store.insert(Type::Int(IntSize::I256)); // TypeId(21)
        store.insert(Type::UInt(IntSize::I256)); // TypeId(22)
        store.insert(Type::Int(IntSize::I512)); // TypeId(23)
        store.insert(Type::UInt(IntSize::I512)); // TypeId(24)
        store
    }
    ...
    pub fn i256(&self) -> TypeId {
        TypeId(21)
    }
    pub fn i512(&self) -> TypeId {
        TypeId(23)
    }
    ...
    pub fn u256(&self) -> TypeId {
        TypeId(22)
    }
    pub fn u512(&self) -> TypeId {
        TypeId(24)
    }
...
pub fn builtin_type_id_for_name(types: &TypeStore, name: &str) -> Option<TypeId> {
    match name {
        ...
        "i256" => Some(types.i256()),
        "i512" => Some(types.i512()),
        ...
        "u256" => Some(types.u256()),
        "u512" => Some(types.u512()),
...
pub fn builtin_type_by_id(id: TypeId) -> Option<Type> {
    match id.0 {
        ...
        21 => Some(Type::Int(IntSize::I256)),
        22 => Some(Type::UInt(IntSize::I256)),
        23 => Some(Type::Int(IntSize::I512)),
        24 => Some(Type::UInt(IntSize::I512)),
        _ => None,
    }
}
...
mod tests {
    ...
    #[test]
    fn builtin_type_lookup_supports_256_and_512_bit_integers() {
        let types = TypeStore::new();
        assert_eq!(builtin_type_id_for_name(&types, "i256"), Some(types.i256()));
        assert_eq!(builtin_type_id_for_name(&types, "u256"), Some(types.u256()));
        assert_eq!(builtin_type_id_for_name(&types, "i512"), Some(types.i512()));
        assert_eq!(builtin_type_id_for_name(&types, "u512"), Some(types.u512()));
    }

    #[test]
    fn builtin_type_reverse_lookup_supports_256_and_512_bit_integers() {
        assert_eq!(
            builtin_type_by_id(TypeId(21)),
            Some(Type::Int(IntSize::I256))
        );
        assert_eq!(
            builtin_type_by_id(TypeId(22)),
            Some(Type::UInt(IntSize::I256))
        );
        assert_eq!(
            builtin_type_by_id(TypeId(23)),
            Some(Type::Int(IntSize::I512))
        );
        assert_eq!(
