...
impl Resolver {
        ...
        match &stmt.kind {
            StmtKind::Let {
                ...
                if let Some(name) = self.pattern_name(pattern)
                    && let Err(prev) = self.scopes.declare(
                        name.clone(),
                        SymbolKind::Variable {
                            mutable: *mutable,
                            ty: resolved_ty,
        ...
        match &expr.kind {
            ...
            ExprKind::PathExpr(path) => {
                if let Some(first) = path.segments.first()
                    && let Some(id) = self.scopes.lookup(&first.name)
                {
                    self.scopes.mark_used(id);
    ...
    fn resolve_type_expr_to_id(&mut self, te: &TypeExpr) -> TypeId {
        ...
        match &te.kind {
            ...
            TypeExprKind::Array { element, size } => {
                let element_id = self.resolve_type_expr_to_id(element);
    ...
    fn builtin_function_signatures(&self) -> Vec<(&'static str, Vec<TypeId>, TypeId)> {
        ...
        vec![
            ...
            ("gpu_malloc", vec![int_ty], any),
            ("gpu_free", vec![any], int_ty),
            ("gpu_memcpy_to_device", vec![any, any, int_ty], int_ty),
            ("gpu_memcpy_to_host", vec![any, any, int_ty], int_ty),
            ("len", vec![any], int_ty),
...
"#,
    ...
    #[test]
    fn test_fixed_array_type_resolves_without_errors() {
        let r = parse_and_resolve("fn kern(input: [f32; 64]): return 0");
        assert!(r.errors.is_empty(), "errors: {:?}", r.errors);
    }
