        ...
        for instr in &block.instructions {
            let inferred = match &instr.op {
                ...
                Op::StoreLocal { name, value } => {
                    ...
                    let ty = if value_ty != LlvmType::default_int() {
                        value_ty
                    } else {
                        layout
                            .local_types
                ...
                let inferred_sign =
                    match &instr.op {
                        ...
                        Op::GetIndex { object, .. } => value_signs
                            ...
                            .copied()
                            .unwrap_or_else(|| default_sign_for_type(value_type(&layout, *object))),
                        Op::StoreIndex { value, .. } => value_signs
                            .get(value)
                            .copied()
                            .unwrap_or_else(|| default_sign_for_type(value_type(&layout, *value))),
                        Op::Phi(entries) => entries
...
fn is_gpu_host_builtin(name: &str) -> bool {
    matches!(
        name,
...
fn has_runtime_helper_definition(name: &str) -> bool {
    matches!(name, "argc" | "argv" | "parse_int") || is_gpu_host_builtin(name)
}
...
fn builtin_arg_types(name: &str) -> Option<Vec<LlvmType>> {
    match name {
        ...
        "gpu_malloc" => Some(vec![LlvmType::default_int()]),
        "gpu_free" => Some(vec![LlvmType::OpaquePtr]),
        "gpu_memcpy_to_device" | "gpu_memcpy_to_host" => Some(vec![
            LlvmType::OpaquePtr,
            LlvmType::OpaquePtr,
            LlvmType::default_int(),
        ]),
...
fn module_uses_gpu_host_builtins(module: &MirModule) -> bool {
...
impl LlvmEmitter {
    ...
    fn emit_module(&mut self, module: &MirModule) -> Result<String, String> {
        ...
        if uses_gpu_host_runtime {
            output.push_str(&emit_host_cuda_declarations());
        }
        ...
        if uses_gpu_host_runtime {
            emit_gpu_host_runtime_helpers(&mut output);
        }
...
fn emit_gpu_host_runtime_helpers(out: &mut String) {
    out.push_str(
        "define noundef i8* @agam_gpu_malloc(i32 noundef %size_bytes) local_unnamed_addr {\n\
         entry:\n\
           %ptr = alloca i8*\n\
           store i8* null, i8** %ptr\n\
           %is_non_positive = icmp sle i32 %size_bytes, 0\n\
           br i1 %is_non_positive, label %return_null, label %alloc\n\
         \n\
         alloc:\n\
           %size64 = sext i32 %size_bytes to i64\n\
           %status = call i32 @cudaMalloc(i8** %ptr, i64 %size64)\n\
           %ok = icmp eq i32 %status, 0\n\
           br i1 %ok, label %load_ptr, label %return_null\n\
         \n\
         load_ptr:\n\
           %loaded_ptr = load i8*, i8** %ptr\n\
           ret i8* %loaded_ptr\n\
         \n\
         return_null:\n\
           ret i8* null\n\
         }\n\n\
         define noundef i32 @agam_gpu_free(i8* noundef %ptr) local_unnamed_addr {\n\
         entry:\n\
           %is_null = icmp eq i8* %ptr, null\n\
           br i1 %is_null, label %return_zero, label %free\n\
         \n\
         free:\n\
           %status = call i32 @cudaFree(i8* %ptr)\n\
           ret i32 %status\n\
         \n\
         return_zero:\n\
           ret i32 0\n\
         }\n\n\
         define noundef i32 @agam_gpu_memcpy_to_device(i8* noundef %device_dst, i8* noundef %host_src, i32 noundef %size_bytes) local_unnamed_addr {\n\
         entry:\n\
           %dst_is_null = icmp eq i8* %device_dst, null\n\
           %src_is_null = icmp eq i8* %host_src, null\n\
           %has_null = or i1 %dst_is_null, %src_is_null\n\
           %size_is_negative = icmp slt i32 %size_bytes, 0\n\
           %is_invalid = or i1 %has_null, %size_is_negative\n\
           br i1 %is_invalid, label %return_error, label %copy\n\
         \n\
         copy:\n\
           %size64 = sext i32 %size_bytes to i64\n\
           %status = call i32 @cudaMemcpy(i8* %device_dst, i8* %host_src, i64 %size64, i32 1)\n\
           ret i32 %status\n\
         \n\
         return_error:\n\
           ret i32 -1\n\
         }\n\n\
         define noundef i32 @agam_gpu_memcpy_to_host(i8* noundef %host_dst, i8* noundef %device_src, i32 noundef %size_bytes) local_unnamed_addr {\n\
         entry:\n\
           %dst_is_null = icmp eq i8* %host_dst, null\n\
           %src_is_null = icmp eq i8* %device_src, null\n\
           %has_null = or i1 %dst_is_null, %src_is_null\n\
           %size_is_negative = icmp slt i32 %size_bytes, 0\n\
           %is_invalid = or i1 %has_null, %size_is_negative\n\
           br i1 %is_invalid, label %return_error, label %copy\n\
         ...
         copy:\n\
           %size64 = sext i32 %size_bytes to i64\n\
           %status = call i32 @cudaMemcpy(i8* %host_dst, i8* %device_src, i64 %size64, i32 2)\n\
           ret i32 %status\n\
         \n\
         return_error:\n\
           ret i32 -1\n\
...
mod tests {
    ...
    #[test]
    fn test_emit_gpu_host_runtime_helpers() {
        let llvm = compile_to_llvm(
            "fn main() -> i32 { let ptr = gpu_malloc(16); gpu_memcpy_to_device(ptr, \"abcd\", 4); gpu_memcpy_to_host(\"wxyz\", ptr, 4); gpu_free(ptr); return 0; }",
        );
        assert!(llvm.contains("declare i32 @cudaMalloc(i8**, i64)"));
        assert!(llvm.contains("declare i32 @cudaFree(i8*)"));
        assert!(llvm.contains("declare i32 @cudaMemcpy(i8*, i8*, i64, i32)"));
        assert!(llvm.contains("define noundef i8* @agam_gpu_malloc"));
        assert!(llvm.contains("define noundef i32 @agam_gpu_memcpy_to_device"));
        assert!(llvm.contains("define noundef i32 @agam_gpu_memcpy_to_host"));
        assert!(llvm.contains("define noundef i32 @agam_gpu_free"));
        assert!(llvm.contains("call noundef i8* @agam_gpu_malloc"));
        assert!(llvm.contains("call noundef i32 @agam_gpu_memcpy_to_device"));
